      ...
      <main className="byid__main">
        <section className="byid__section-1">
          ...
          <div className="p__container">
            <p className="byid__p">
              <span className="byid__p-span">Weight</span>
              {pokemon?.weight}
            </p>
            <p className="byid__p">
              <span className="byid__p-span">Height</span>
              {pokemon?.height}
            </p>
          </div>
          <div className="byid__list">
            ...
            <ul className="byid__ul-type">
              {pokemon?.types.map((type) => (
                <li
                  key={type.type.url}
                  className={`byid__list-item item__type bg-${type.type.name} `}
                >
                  {type.type.name}
                </li>
              ))}
            ...
            <ul className="byid__ul-ability">
              {pokemon?.abilities.map((ability) => (
                <li key={ability.ability.url} className="byid__list-item">
        ...
        <section className="byid__section-2">
          ...
          <ul className="movements__container">
            {pokemon?.moves.map((move) => (
              <li className="movements">{move.move.name}</li>
            ))}
          </ul>
        </section>
      </main>
